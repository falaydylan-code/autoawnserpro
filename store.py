...
def reserve_plan(owner, run_id, request_id, question, phase, cap, amount, model, budget_owner=None):
    ...
    with connect() as con:
        ...
        if count >= ({'repair':2,'plan':3,'inspection_correction':1,'format_correction':1,'visual':125,'verify':4,'navigation':2}.get(phase,0)):raise ValueError('BUDGET_EXHAUSTED: question model-call budget reached.')
        import time
