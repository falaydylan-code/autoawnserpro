...
## 2026-09-24 - claude (kimchi, integrator) - 0.10.48 merged and deployed: answers-in-place after Check + page-mode text + Khan course pager

**Did:** On Dylan's "ship the fix including mira's updates": fast-forwarded codex/optional-numbering 17f2e3a -> 1b01c85 (origin had nothing new). Release = 764c970 (kimchi: after our own Check the question is judged by the answers still in the same boxes; the page-mode class words grade/score/saved/attempt never erase an element that holds answers -- McGraw's "pregrade-mode" body) + 3651087 via 1b01c85 (Mira: Khan's "Previous in course"/"Next in course" pager recognised as workflow chrome after stripping the shared phrase). Deployed backend-first (only app.py differed: version; frontend identical; local capabilities 0.10.48); live `/api/capabilities?protocol=4` = 0.10.48 on the first poll, `/api/health` ok. Pushed to origin.

**Verified:** Full suite on the shipped tree: 536 passed + 1 `TargetClosedError` (`test_greptile_pr6` same-site navigation) that passed alone in 41 s -- the known crash flake. Pager + transitions files together 25 passed.

**Left undone:** Live reruns: Ch.3 Q1 (expect past Check; the graded page greys out Next, likely the next stop) and Khan. Khan "fix 1" (a discovered group must not block the finish gate at planner_runtime.js ~:920 once >=1 answer is verified) NOT built -- awaiting Dylan's go to kimchi; Mira's `test_a_pager_whose_words_are_absent_still_gets_through` pins today's needs_review and flips to finished when it lands. Formative ship blocker still partly open.

**Watch out:** New base for every branch: 1b01c85. `reachDeclaredParts` still moves ledger records the old way on re-key. The coverage rule lives in planner.py AND planner_runtime.js.

## 2026-09-24 - claude (kimchi, integrator) - after our own Check, the question is judged by the answers, and a page-mode class never erases the question (0.10.48)

**Did:** Live Ch.3 Q1 on 0.10.47 (7:10 PM): past the frame fix, stopped again after a correct graded answer, `TARGET_STALE ... after Check (stem)`; failure_data showed the new stem hash `811c9dc5` = hash(''). Diagnosed read-only on the Ch.3 STUDY copy (no clicks on answers or Check): the accounting-tool frame has no `<main>`, so the harness's question root is `<body>`; McGraw's own tool code runs `case EZ.MODE_PREGRADE: $("body").addClass("pregrade-mode")` on Check; `[class*=grade]` in `textExcluded` (`planner_content.js` :62) matched the ROOT, so `renderedText` returned nothing and the whole identity stem vanished. Built on Dylan's go, two halves, each sufficient on its own for the McGraw case: (1) `planner_content.js` renderedText only (Mira's file, her yes with three conditions, all taken): the page-MODE words grade/score/saved/attempt drop an element's text only when it holds no answer control; feedback/result/correct stay unconditional (a reveal box beside a retry input must never put the answer key into question text); the answer-control test runs only after a mode word matched; every other reader of `textExcluded` keeps the full list. (2) `planner_runtime.js` `answersStillInPlace()`: after OUR Check, if the key moved, the page is the same question when every verified box that is visible still holds its verified answer (matched by part + element key, via the existing `matches()`), with at least three (or all) visible; then `current.key` follows and the documents are re-pinned. Otherwise the existing stop, now saying why (`answers not proven in place: ...`) with `answers_in_place` in failure_data. Found and fixed in my own first cut: graded pages that lose their text make DIFFERENT questions share one key, and moving each question's ledger record to that key made each overwrite the last (3 finished -> 1 record); the record now stays under the key the question started with. Versions 0.10.48.

**Verified:** Fixture modes in `transitions.html`/`transitions_q.html`: `pregrade` + `bare` (no `<main>`, root = body, as live), `banner` (unrecognised verdict), `wipe`, `wipebanner`, `verdictonly`, `different`. My first pregrade fixture kept `<main>` and passed on the OLD code -- rebuilt with `bare=1` to match McGraw's structure, now fails on 0.10.47 exactly as live and passes on 0.10.48. Each half alone passes pregrade. Against 0.10.47: pregrade, banner, reworded, and wiped-and-changed all fail. The record-overwrite test fails on the buggy re-key (1 record) and passes on the fix (3). Transitions 18/18. Full suite **530 passed**, clean.

**Left undone:** Live rerun of Ch.3 Q1 (next expected obstacle: the graded page greys out Next). The grade banner's own markup was never seen (would need pressing Check live). Not merged, not deployed, not pushed. Khan finish-gate "fix 1" (discovered groups non-fatal at `:920` when >=1 answer verified) awaits Dylan's go to kimchi; Mira's fix 2 on claude/khan-pager is separate.

**Watch out:** Two EXISTING expectations moved on purpose: `check=stem`/`check=rebuildstem` (same question reworded, answer still in the box) now CONTINUE; the "different question stops" protection moved to `check=different`. A page that only clears its boxes keeps its fingerprint (typed values are not in it), so nothing is judged and the run carries on as before. `reachDeclaredParts` (0.10.46) still MOVES ledger records on re-key the old way -- the same overwrite is possible there if two asserted-part questions share a key; not changed here.
## 2026-09-24 - claude (mira, site session) - a known control wrapped in the site's own wording is still that control

**Did:** Khan Academy (7:34 PM, 0.10.47) chose Choice D correctly, verified it by DOM and by a confirming screenshot, and then ended `QUESTION_INCOMPLETE: Answer controls remain unresolved` (`planner_runtime.js:920`). The leftover named in `failure_data` was the LEFT SIDEBAR's course pager -- two plain buttons, "Previous in course" and "Next in course" -- discovered as a candidate answer group, unclassifiable because its own scope text is just the lesson title, and an unresolved leftover fails the finish gate. Not caused by the Formative bundle: the pair was discovered, unresolved and fatal on 0.10.46 too. It is the whole-group chrome rule shipped in 0.10.47 that should have caught it and did not -- that rule needs EVERY label to be a recognised workflow word, and "Next in course" matches nothing, because the advance pattern matches a whole label ("Next", "Next question", "Next part", "Continue") and not a phrase inside one. Fix, `planner_content.js` `discoverChoices` only: test the workflow vocabulary against the RESIDUAL left after stripping the phrase the group's labels share, reusing the `residuals()` helper the mirror rule already uses. "Previous in course"/"Next in course" strips to "Previous"/"Next", both already recognised; "Go to previous question"/"Go to next question" strips through the prefix instead. No wording was added to any list -- `chromeLabel`, `navigationKind` and `auxiliaryGroup` are byte-identical. Fixture `tests/fixtures/course_pager.html` (Khan shape, no `data-question-id` so the root is `<main>`, pager NOT in `nav`/`aside` because the live one was discovered; `?pager=wrapped|plain|unworded|marked|none`, `?answers=opposite`); `tests/test_planner_course_pager.py` (7).

**Verified:** New file 7 passed. Regression on this branch: choice_mode + discovery + discovery_runtime + discovery_contract + tool_rows 56; interactions + result_cards + visual_readback + contract 73; executor + acceptance + parts + worksheet_tabs + navigation 109. 245 focused tests green. Also unit-checked the rule's logic outside the browser against nine label sets: the Khan pager, a plain pager and a shared-prefix pager are chrome; Khan's real options, Formative's options and generic "Choice A/B/C" are not. Proven both ways in the fixture: `?pager=marked` (pager carries `aria-pressed`) is KEPT, and `?answers=opposite` ("Previous year"/"Next year" as real options) is KEPT -- markup outranks labels in both directions. FULL SUITE NOT RUN: the box had 2.8 GB free against the 4 GB a full run needs (62 Chrome processes, all Dylan's own browser, no test leftovers to reap).

**Left undone:** The full suite. The finish-gate half ("fix 1"), which is the general answer and is NOT this change: `planner_runtime.js:906-920` draws no line between a control the PAGE marks up as an input -- which genuinely might be unanswered and must stay fatal -- and a group the HARNESS guessed at from repeated buttons, which is our own speculation. The same line already exists in `planner.py` validate_context and in `validate()` since 56d4735; carrying it to the finish gate would let a correctly answered question finish whatever the wording, and should be non-fatal only when at least one answer was entered AND verified, still listed in `not_answerable`. Waiting on Dylan's direct go to kimchi. Version not bumped.

**Watch out:** This rule is NOT vocabulary-free and the tests say so on purpose. `?pager=unworded` ("Forward in course"/"Backward in course") is asserted to still get through, still go unresolved, and still end a correctly answered question -- `test_a_pager_whose_words_are_absent_still_gets_through` expects `needs_review`, and that one assertion is designed to FLIP to `finished` when the finish-gate fix lands. It is the only expectation of mine that should move. Second known false positive, also pinned: a real answer group reading "Previous year"/"Next year" strips to workflow words and would be withheld on labels alone -- only the page's own markup saves it, so a site with bare-button answers and opposite-word options has neither protection. A bare "Previous"/"Next" pair never forms a group at all ("Next" is filtered one control at a time by the pool), so nothing is recorded as suppressed for it -- absence of a suppression record is not absence of the fix.

## 2026-09-24 - claude (kimchi, integrator) - 0.10.47 merged and deployed: frame identity + Formative bundle (part) + runtime mirror

**Did:** On Dylan's "ship it": fast-forwarded codex/optional-numbering 9dd39ce -> 166c290 (origin had nothing new). Release = 09b16cc (kimchi: Chrome's frame number leaves identity), 407854b via 70f39e4 (Mira: tool rows are not answer groups, discovered groups optional, role before type, chrome pairs as whole groups, gid without trailing segment), 56d4735 (kimchi: runtime mirror of the optional rule). Deployed backend-first from a refreshed deploy-source (app.py + planner.py differed; frontend identical; local capabilities 0.10.47); live `/api/capabilities?protocol=4` = 0.10.47 on the first poll, `/api/health` ok. Pushed to origin.

**Verified:** Integrated full suite 525 passed, clean, on exactly the shipped tree. Live handshake after deploy.

**Left undone:** Live reruns -- Ch.3 Q1 (frame fix: expect it to get past Check; if it stops, the log now names every mover, and `structure` would point at the graded page's checkmarks; Next was greyed on the graded page) and a Formative question (tool rows). The Formative ship blocker is PARTLY built: still open are framework ids out of fingerprint and key()/slotKey(), question text cut by candidate discovery, one-label choose_one<->set_choice_set aliases, duplicated option text. Mira's `suppressed_groups` is not surfaced in the Copy log (one runtime line, outside the go).

**Watch out:** New base for every branch: 166c290. Slot keys no longer carry a frame segment. The coverage rule lives in both planner.py validate_context and planner_runtime.js validate() -- change both together.

## 2026-09-24 - claude (kimchi, integrator) - the runtime honours "a discovered group is optional", or the Formative fix does nothing live

**Did:** Merged Mira's Formative bundle (407854b) into 0.10.47 as 70f39e4, then found on review that its central rule only existed on one side. `planner.py` validate_context now lets a plan leave out a group the harness DISCOVERED (adapter `candidate_choices`), but `planner_runtime.js` keeps its own copy of the coverage rule, and Mira (correctly) stayed out of that file. Proven on the merged tree with the bundle's own fixture in its non-mirror mode (`tool_rows.html?tools=distinct&chrome=none`, where the "Cross out" row survives as a discovered group) and a model that plans only the real answer: first `QUESTION_INCOMPLETE: Plan does not cover every answerable slot` (runtime `validate()` required every resolved slot); with that mirrored, `Cannot execute an incomplete plan` -- the round loop took the skipped group for "an answer control that appeared after the planned answers", re-offered it alone, the model skipped it again, and the empty plan was refused. Fixed in `validate()`: coverage is required of every resolved slot EXCEPT discovered ones, while membership still accepts any resolved slot (so planning the discovered group too stays allowed); discovered groups offered and left out are recorded in `current.optionalSkipped`, and the round loop does not re-offer them. The bundle's own engine test could not see either gap: its mirror rule removes the tool row before the model is shown it, so the optional path never ran end to end there.

**Verified:** New `tests/test_planner_optional_groups.py` (4): the page offers one marked-up answer and one discovered group; leaving the discovered group alone finishes with ONE plan call and the tool row untouched; planning it too is still accepted; leaving out the marked-up answer is still refused with nothing clicked. Against the pre-fix runtime (09b16cc's file on the merged tree) the behaviour test fails and the two guard tests pass -- they exist to prove the fix did not over-loosen. Full suite on the integrated tree (frame identity + Formative bundle + this mirror): **525 passed**, clean.

**Left undone:** Live Formative rerun. Mira's `suppressed_groups` field (why a group was not offered) is not printed in the Copy log or events -- a one-line runtime change, outside the current go; raised with Dylan.

**Watch out:** The runtime and backend each carry a copy of the coverage rule (`planner_runtime.js` validate, `planner.py` validate_context). Change one, change both, and test with a group the model actually leaves out -- a fixture where the harness removes the group first proves nothing about this rule.

## 2026-09-24 - claude (kimchi, integrator) - a rebuilt question frame is the same question: Chrome's frame number leaves identity (0.10.47)

**Did:** First live run of 0.10.46 (McGraw Connect Ch.3 Q1, 12:10 AM): both 0.10.46 fixes worked -- twelve cells across two tabs entered and verified, the page graded it "Answer is complete and correct", and the navigation layer found "Check my work" and pressed it. Then `TARGET_STALE: Question identity changed after Check (frames [1747] -> [1752])`. McGraw grades by DESTROYING the question iframe and inserting a new one; Chrome numbers the new element afresh; that number was inside the question key (`planner_runtime.js` :151) and every slot key (:155), so the same graded question read as a different one. The identity guard is right and is kept -- its input was wrong. Built on Dylan's go, after reviewing a proposal another model wrote: (1) the question key is built from each frame's own key only (which already holds its address, question text and answer layout, `planner_content.js` :444); slot keys drop the frame segment (now `question_key/<frame-local key>`); the ledger's "never press Next twice" signature uses the frame's key -- it is stored and compared later. Frame ids remain for routing and the log only. (2) `identity.frames` names a frame by its ADDRESS (origin + path + hash, the parts its own key reads; `#n` when two frames share one), so "frames moved" means a frame appeared or left, not that its content changed. (3) `identityMovers` reports every mover instead of returning at the first -- the 12:10 AM log said only "frames" and nobody could tell whether the question text had also moved on the graded page. (4) Duplicate-slot message now says two question areas look identical. (5) The Copy log includes `failure_data`. From the proposal, deliberately NOT built: its label-bearing identity name for unnamed elements (would have re-broken Q12, whose tab `part:node:2` carries "not yet entered" in its aria-label -- the exact 0.10.45 bug); my label-free replacement for it (positional index via raw `querySelectorAll` counts result icons the harness excludes, so a checkmark appearing after a correct click moves an unnamed answer's key while the question key holds -- reverted before commit; McGraw names every tab and cell with an id, so nothing needed it); its table-dedup change (would add a field to tables sent to the model, which `SourceTable(Strict)` forbids -- every plan with a table would fail; that dedup is in-memory within one pass anyway); an overlay wait and a measurement reset (no log shows either; the reset added a field nothing reads and would discard valid geometry at `reachDeclaredParts`' repin). Versions 0.10.47.

**Verified:** New fixture modes in `transitions.html` -- `check=rebuild`, `check=rebuildstem`, `next=rebuild` -- REPLACE the iframe element (the old `check=reload` navigates the same element, which keeps its frame id; that is why it passed while the real site failed). Three new tests in `test_planner_transitions.py`, proven both ways: on the unfixed runtime all three fail, the rebuild case with `frames [2] -> [3]` (the live message); on the fix, Check-rebuild and Check+Next-rebuild complete all three questions with every graded page adopted, and a rebuild holding a DIFFERENT question still stops with nothing entered and now names `stem`. First full run caught my own first draft (frames named by content fingerprint turned every stem change into "frames ..., stem"); fixed by naming frames by address. Final full suite **506 passed**, clean.

**Left undone:** Live rerun of Ch.3 Q1. Expect the next stop, if any, to be on the graded page's own content -- McGraw adds green checkmarks after Check; if those count as answer layout the log will now say `structure` (not `frames`), which is a separate fix. The screenshot also shows Next greyed out on the graded page. Not merged, not deployed, not pushed.

**Watch out:** Slot keys have a new shape (no frame segment); nothing parses them by position (checked). Two question areas that are truly identical -- same address, text and answer layout -- now collide and stop `TARGET_AMBIGUOUS` instead of being told apart by frame number; rare, and stopping is right. A frame whose address changes on grading (not just its query string -- the key reads path + hash) would still move the key; the log will say `frames` if so.
## 2026-09-24 - claude (mira, site session) - a per-option tool row is not an answer group, and a discovered group is never mandatory

**Did:** Formative macroeconomics quiz (1:00 AM, 0.10.46) answered correctly and the plan was thrown away: `QUESTION_INCOMPLETE: plan must cover every resolved offered slot`. Beside every option Formative puts a second button, "Strikethrough this option - <that option>". Those four are repeated siblings with unique labels in a scope whose text settles pick-one, so `planner_content.js` typed them `kind = g.mode` -- a RESOLVED slot -- and `planner.py` required every resolved slot to be planned, i.e. required the model to plan crossing out its own correct answer. It planned only the real answer, which was right, and the run died before any click. Both lines predate 0.10.46 (discovered-kind since 0.10.38, the coverage rule since 0.10.40). Four changes on Dylan's go, all general, none keyed to a hostname, class or site string: (1) `planner.py` validate + prompt: a slot whose `interaction.adapter` is `candidate_choices` is OPTIONAL -- the harness discovered it, so it is a guess. Slots the page marks up (value, selection, native choice/choice_set) stay required. Optional never means empty: a task-less plan is already refused as SCHEMA_INVALID server-side and again at `planner_runtime.js:780`, so no third guard was added (I wrote one, found it dead, removed it). (2) MIRROR rule in `discoverChoices`: strip the phrase a group's labels share; if what is left maps one for one onto ANOTHER group's labels as that group writes them, it is a tool row for that group. Prefix and suffix are tried separately -- the live options share the tail "unemployment", so stripping both at once mirrors nothing. The target must match literally, so two tool rails cannot cancel each other out. (3) Whole-group chrome rule: a group whose every label is workflow is page chrome. Formative's Previous/Next and hint/Feedback pairs each arrived as a candidate group because the pool filter judges ONE control at a time. It reuses the vocabulary the navigation path already owns -- `chromeLabel`, hoisted out of `navigationInfo` so both read one list -- plus the auxiliary words, and markup outranks labels: any member the page marks up as an answer control keeps its group. Done in the content script, not at `planner_runtime.js:890`, deliberately: the group never becomes a slot, so the finish gate never sees it, and kimchi's file stayed untouched. (4) ARIA role outranks `e.type`: `<button type="button" role="radio">` is pick-ONE. The live group was typed choice_set, which would have let the harness enter several answers on a pick-one question -- a wrong-answer risk, not just a stop -- and its key carried an empty trailing segment (`button:node:45:`); the name segment is now written only when a radio group has a name. Suppressed groups stay candidates (their text is still kept out of the stem) and are reported on the frame observation as `suppressed_groups` with the reason. Fixture `tests/fixtures/tool_rows.html` (option rail + tool rail + aid and pager pairs; `?tools=none|distinct|partial|only`, `mirror=both`, `chrome=none|answer`, `plain=1`); `tests/test_planner_tool_rows.py` (15).

**Verified:** New file 15 passed. Regression on this branch: contract + discovery_contract 54; choice_mode + discovery + discovery_runtime + result_cards + visual_readback + interactions 60; executor + acceptance + parts + worksheet_tabs + navigation 109. 238 focused tests green in total. Proven both ways on the fixture: the strikethrough rail is withheld and the answer still enters (`window.struck` stays empty); a rail that shares a phrase but mirrors nothing ("Cross out A".."D") stays an answer group; a three-tool rail against four options is not a mirror; with the options as plain text the rail is the only choice-like group and is never withheld; two rails both mirror the real group and both go; options with no shared tail mirror through the prefix; a pager pair marked `role=radio` survives its labels. FULL SUITE NOT RUN: kimchi's was already running (pid 25908, started 10:56) and two Playwright suites at once kill this box. NOT live-tested on Formative.

**Left undone:** The full suite, and the live Formative rerun (Dylan). Two things seen in the same log and deliberately not fixed here, to keep one change one change: the real group's LABEL is polluted with the tool text ("Seasonal unemployment strikethrough_s Cyclical ...") because `name(group)` reads the container's innerText and does not exclude candidate members -- cosmetic, the `options` the model reads were correct; and the framework-id item of the Formative bundle (React ids out of `key()`/`slotKey()`) is untouched. Version NOT bumped, per kimchi: she decides at merge whether this rides in 0.10.47 with her frame-identity fix or goes out as 0.10.48.

**Watch out:** Two words already in the `auxiliary` list -- notably "flag" -- remove a control from the candidate pool one member at a time, so a tool rail labelled "Flag - X" never reaches the mirror rule at all (found while writing the fixture; the second rail is "Highlight - X" for that reason). The chrome rule is skipped entirely for a group where any member carries `[role=radio]`, `[role=checkbox]` or `[aria-pressed]`, which is what keeps a real aria-pressed answer group safe -- do not widen it to those. `suppressed_groups` is recorded on the frame observation but the Copy log does not print it: surfacing it needs one line in `planner_runtime.js` (kimchi's file), worth adding when that file is next open. `9dd39ce` rewrote all nine extension files LF->CRLF with zero content change; do not set `core.autocrlf=false` in a worktree, it makes every file look modified.
