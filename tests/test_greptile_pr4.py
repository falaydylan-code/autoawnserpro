...
def test_one_revision_is_taken_and_reported_and_the_second_stops(extension):
    """Live run: the model garbled one field of a twenty-part re-read and the
    plan-change guard ended the whole run. A first change is a correction or a
    slip; either way the part is reset and re-entered. A second is flip-flop."""
    page, w, tid = navigate(extension, 'custom_dropdowns.html')
    result = w.evaluate('''async id=>{const h=__assignmentHarness;const p=await h.observeAllFrames(id);
      const cell=p.elements.find(e=>e.key.endsWith('#cell_6_1')).ref;
      const read=answer=>h.coverage.read({question:'Classify accounts',parts:[{id:'re_stmt',what:'Retained Earnings - Statement',answer,ref:cell}]},p);
      read('Balance Sheet');
      read('Retained Earnings');const first={revised:[...h.coverage.revised],answer:h.coverage.ledger()[0].answer};
      try{read('Income Statement');return {first,second:'accepted'};}catch(e){return {first,second:e.message};}}''', tid)
