...
def test_extension_waits_as_long_as_the_backend_advertises_and_names_the_wait_when_it_gives_up(extension):
 ...
 result=w.evaluate("""async id=>{
   ...
   globalThis.fetch=async(u,opts={})=>{u=String(u);
     if(u.includes('/api/capabilities'))return new Response(JSON.stringify({protocol:4,request_wait_seconds:240,features:['task_plans','stable_slots','typed_verification','bounded_repair','frame_scoped_inspection','interaction_classification','choice_discovery','bounded_format_correction','contextual_navigation']}));
